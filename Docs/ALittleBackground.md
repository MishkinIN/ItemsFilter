...
Если такой фильтр может потребоваться для каждого столбца, отображаемого в `DataGrid`, это становится проблемой, 
...
При указании коллекции в качестве источника привязки WPF не выполняет прямую привязку к коллекции. 
Вместо этого WPF фактически привязывается к представлению коллекции по умолчанию `CollectionViewSource`, 
...
созданный ранее для этого source `FilterPresenter`.
...
и отражены возможности, предоставляемые библиотекой `ItemsFilter`. Каждый из приёмов внедрения описан
